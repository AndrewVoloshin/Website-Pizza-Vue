<template>
  <div class="checkout">
    <div class="checkout__container">
      <div class="order__placed"
           v-if="orderPlaced">
        <app-title :titleText="'order placed'"
                   class="title" />
      ...
      <div class="order__checkout"
        ...
        <div class="checkout__section">
          ...
          <div v-if="$v.address.$invalid && !isInput"
               class="checkout__error address__add">
            <strong>No Address Found</strong>
          </div>
          <button v-if="!isInput && $v.address.$invalid"
                  @click="isInput = true"
                  class="checkout__btn address__btn">Add Address</button>
          <button v-if="!isInput && !$v.address.$invalid"
                  @click="isInput = true"
                  class="checkout__btn address__btn">Update Address</button>
          <div class="form"
               v-if="isInput">
            <input v-model.trim="address.buildingNumber"
                   type="text"
                   class="checkout__input"
                   placeholder="Building Number" />
            <input v-model.trim="address.streetName"
                   type="text"
                   class="checkout__input"
            ...
            <input v-model.trim="address.state"
                   ...
                   class="checkout__input"
                   placeholder="State" />
            <input v-model.trim="address.country"
                   type="text"
                   class="checkout__input"
                   placeholder="Country" />
            <input v-model.trim="address.pinCode"
                   type="text"
                   class="checkout__input"
                   placeholder="Pin Code" />
            <div v-if="$v.address.$anyError"
                 class="checkout__error address__valid">
              <strong>Please Enter a valid address</strong>
            </div>
            <button @click="cancel"
                    class="checkout__btn cancel">Cancel</button>
            <button @click="update"
                    class="checkout__btn update">Update</button>
          </div>
        </div>
        <div class="checkout__section">
          <div class="checkout__subtitle">
          ...
          <form class="checkout__form">
            ...
            <div v-if="$v.$anyError && $v.$anyDirty"
                 class="checkout__error order__error">
              <strong v-if="$v.address.$invalid && !$v.payment.required">Please make sure that all fields are
                filled</strong>
              <strong v-else-if="$v.address.$invalid && $v.payment.required">Please fill in the address field</strong>
              <strong v-else-if="!$v.address.$invalid && !$v.payment.required">Please select the mode of payment
                field</strong>
            </div>
            <button class="checkout__btn order"
                    @click.prevent="placeOrder">Place Order</button>
          </form>
        </div>
      </div>
    </div>
