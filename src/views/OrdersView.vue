<template>
  <div class="orders">
    <div class="orders__container">
      <app-title :titleText="'orders'"
                 class="title" />
      <h1 v-if="!$store.state.ordersHistory.length">You haven't ordered anything yet!</h1>
      <div class="orders__content"
           v-if="$store.state.ordersHistory.length">
        <div class="line"></div>
        <div class="order__section"
             v-for=" (order, index) in $store.state.ordersHistory"
             :key="index">
          ...
          <table class="order__table">
            ...
            <tbody v-for="(position, index) in order.orderProduct"
                   :key="index">
              <tr>
                <th>{{ index + 1 }}</th>
                <td>{{ position.name }}</td>
