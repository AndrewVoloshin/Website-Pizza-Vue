<template>
  <div :class="['card', { 'card_mode-order': ismodeOrder }]"
       @mouseover="isMouseOver = true"
       @mouseleave="isMouseOver = false">
    <div :class="['card__img', { 'card__img_mode-order': ismodeOrder }]"
         v-if="!notDiplayImg">
      <img :class="{ card_hover: isMouseOver }"
           :src="item.img"
           :alt="item.imgSrc" />
    </div>
      ...
      <div class="card__info">
        ...
        <div class="info__action">
          <button class="info__btn"
                  @click="changeOrder(-1)"><i class="minus"></i></button>
          <span class="info__result">
            <strong> {{ item.order }} </strong>
          </span>
          <button class="info__btn"
                  @click="changeOrder(1)"><i class="plus"></i></button>
        </div>
      </div>
    </div>
  </div>
