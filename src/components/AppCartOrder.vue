<template>
  <div class="cart-order">
    <h1>Cart</h1>
    <button v-if="$store.state.valueGeneralOrder"
            @click="clear()"
            class="btn__clear">Clear Cart</button>
    <div :class="['cart-order__conteiner', { 'order': $store.state.valueGeneralOrder }]">

      <div v-if="!$store.state.valueGeneralOrder"
           class="cart-order__content">
        <span>Cart is Empty!</span>
      </div>

      <div class="menu__container">
        <app-menu-card class="app-card"
                       :displayBorder="true"
                       :notDiplayImg="true"
                       :item="item"
